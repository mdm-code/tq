...
func TestLexerScan(t *testing.T) {
	...
	want := []Token{
		{Dot, &l.buffer, 0, 1, 0},
		{ArrayOpen, &l.buffer, 1, 2, 1},
		{String, &l.buffer, 2, 11, 11},
		{ArrayClose, &l.buffer, 11, 12, 11},
		{ArrayOpen, &l.buffer, 12, 13, 12},
		{ArrayClose, &l.buffer, 13, 14, 13},
		{ArrayOpen, &l.buffer, 14, 15, 14},
		{Integer, &l.buffer, 15, 16, 16},
		{ArrayClose, &l.buffer, 16, 17, 16},
	}
	have := []Token{}
	for l.Scan() {
	...
	cases := []struct {
	...
	}{
		{
			...
			want: []Token{
				...
				{ArrayOpen, nil, 2, 3, 2},
				{String, nil, 4, 13, 13},
				{ArrayClose, nil, 14, 15, 14},
				{ArrayOpen, nil, 15, 16, 15},
				{ArrayClose, nil, 16, 17, 16},
		...
		},
		{
			...
			want: []Token{
				{Dot, nil, 0, 1, 0},
				{Whitespace, nil, 1, 2, 2},
				{ArrayOpen, nil, 2, 3, 2},
				{Whitespace, nil, 3, 4, 4},
				{String, nil, 4, 13, 13},
				{Whitespace, nil, 13, 14, 14},
				{ArrayClose, nil, 14, 15, 14},
				{ArrayOpen, nil, 15, 16, 15},
				{ArrayClose, nil, 16, 17, 16},
				{ArrayOpen, nil, 17, 18, 17},
				{Whitespace, nil, 18, 19, 19},
				{Integer, nil, 19, 20, 20},
				{Whitespace, nil, 20, 21, 21},
				{ArrayClose, nil, 21, 22, 21},
				{Whitespace, nil, 22, 23, 23},
			},
		},
	}
	for _, c := range cases {
